...
function gerarPix($valor, $desc) {
    ...
    return ['ok'=>false, 'erro'=>$r['message']??'Erro'];
}
...
while (true) {
    ...
    foreach ($d as $at) {
        ...
        if (isset($at['callback_query'])) {
            ...
            if (str_starts_with($data, 'plano_')) {
            ...
            elseif (str_starts_with($data, 'val_')) {
                list(, $op, $valor) = explode('_', $data);
