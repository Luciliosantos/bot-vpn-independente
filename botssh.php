...
// ==============================================
// 🔒 DADOS — TUDO AQUI, SEM ARQUIVO EXTERNO!
// ==============================================
    ...
    foreach ($dados['result'] as $at) {
        ...
        if (isset($at['callback_query'])) {
            ...
            if (strpos($data, 'op_')===0) {
                ...
                enviar(['chat_id'=>$cid, 'text'=>'💰 Valores disponíveis:', 'reply_markup'=>json_encode(['inline_keyboard'=>array_chunk($botoes,2)])]);
                continue;
            }
        ...
        elseif ($txt==='Comprar SSH') {
            ...
            enviar(['chat_id'=>$cid, 'text'=>'🛒 Escolha seu plano:', 'reply_markup'=>json_encode(['inline_keyboard'=>$botoes])]);
        }
        ...
        elseif ($txt==='Ajuda') {
            enviar(['chat_id'=>$cid, 'text'=>'ℹ️ <b>AJUDA</b>\n\n🛒 Comprar → Escolher → Pagar → Receber\n🎁 Teste → 1/dia\n📱 Recarga → Preencher → Processo manual\n\nDúvidas? Fale com o administrador.', 'parse_mode'=>'html']);
        }
    }
    sleep(1);
}
