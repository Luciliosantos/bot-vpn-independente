...
function detectarPainel() {
    ...
    foreach ($portas as $porta) {
        ...
        if ($codigo === 200 || strpos($resp, 'inbound') !== false) {
            $detectado = ['url' => $url, 'porta' => $porta, 'usuario' => 'admin', 'senha' => 'admin', 'inbound' => 1];
            file_put_contents($arquivo_config, json_encode($detectado));
            break;
        }
    }
    if (!$detectado) {
        $detectado = ['url' => 'http://127.0.0.1:54321', 'porta' => 54321, 'usuario' => 'admin', 'senha' => 'admin', 'inbound' => 1];
        file_put_contents($arquivo_config, json_encode($detectado));
    }
    return $detectado;
}
...
function logarPainelAuto() {
    ...
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(['username'=>$p['usuario'],'password'=>$p['senha']]));
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
function salvarNaPastaPainel($login, $senha, $dias) {
    ...
    $contas = file_exists($arquivo_contas) ? json_decode(file_get_contents($arquivo_contas), true) ?: [] : [];
    $expira = time() + ($dias * 86400);
    $contas[] = ['login'=>$login,'senha'=>$senha,'dias'=>$dias,'expiracao'=>$expira,'data_criado'=>time(),'ativo'=>true];
...
function criarContaAuto($dias) {
    ...
    if (logarPainelAuto()) {
        ...
        if ($dados && !empty($dados['obj'])) {
            ...
            $settings['clients'][] = ['id'=>$login,'password'=>$senha,'email'=>$login.'@painel','expiryTime'=>$expira,'enable'=>true];
            $ch2 = curl_init($p['url']."/panel/inbounds/update/".$p['inbound']);
            curl_setopt($ch2, CURLOPT_POST, true);
            curl_setopt($ch2, CURLOPT_POSTFIELDS, http_build_query([
                'id'=>$p['inbound'],'settings'=>json_encode($settings),'up'=>$obj['up']??0,'down'=>$obj['down']??0,
                'total'=>$obj['total']??0,'remark'=>$obj['remark']??'','enable'=>$obj['enable']??true,
                'expiryTime'=>$obj['expiryTime']??0,'port'=>$obj['port']??0,'protocol'=>$obj['protocol']??''
            ]));
    ...
    return ['login'=>$login,'senha'=>$senha,'expira'=>date('d/m/Y', $expira)];
}
...
while (true) {
    ...
    $parametros = http_build_query(['offset'=>$ultimo_id+1,'timeout'=>15,'allowed_updates'=>json_encode(['message','callback_query'])]);
    ...
    foreach ($dados['result'] as $at) {
        ...
        if (isset($at['callback_query'])) {
            ...
            if (strpos($data, 'op_') === 0) {
                ...
                enviar(['chat_id'=>$cid,'text'=>"💰 Valores — $nome_op:",'reply_markup'=>json_encode(['inline_keyboard'=>$botoes_valores])], true);
                continue;
            }
            ...
            if (strpos($data, 'val_') === 0) {
                ...
                if (!isset($operadoras[$op_chave])) continue;
                $sessao[$cid] = ['op'=>$op_chave,'valor'=>(int)$valor_escolhido,'etapa'=>'digitar_numero'];
        ...
        if (isset($sessao[$cid]['etapa']) && $sessao[$cid]['etapa'] === 'digitar_numero') {
            ...
            $pagamentos_pendentes[$uid] = ['mp_id'=>$pix['id'],'tipo'=>'recarga','numero'=>$num,'op'=>$op_chave,'nome_op'=>$nome_op,'valor'=>$valor,'tempo'=>time()];
            enviar(['chat_id'=>$cid,'text'=>"💳 <b>PAGAMENTO VIA PIX — RECARGA</b>\n\n📱 Número: <code>$num</code>\n📶 Operadora: $nome_op\n💰 Valor: R$ ".number_format($valor,2,',','')."\n\n📋 Copie e cole:\n<pre>{$pix['pix']}</pre>\n✅ Após pagar, processamos automaticamente!\n⌛ Pode demorar até 8h.",'parse_mode'=>'html'], true);
            unset($sessao[$cid]);
            continue;
        }
